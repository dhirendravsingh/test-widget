...
import { useStoryVideosModalContext } from "./StoryVideosModalContext";
...
export const useStoryViews = () => {
  ...
  const { storySessionToken } = useStoryVideosModalContext();
  ...
  const flushSync = () => {
    ...
    try {
      ...
      console.log(`%cFailed to flush views cleanup${error}`, "color: red;");
    } finally {
      viewsQueue.current = {};
      sessionStorage.setItem("sessionTime", Date.now().toString());
    }
